...
def test_image_text(sample_data):
    ...
    Xs = [
        pd.DataFrame(Xs[0]),
        pd.DataFrame(["docs/figures/graph.png"]*len(Xs[0])),
...
def test_getitem(sample_data):
    ...
    for i in range(len(dataset)):
        ...
        assert isinstance(sample, tuple)
        assert len(sample) == 3
        Xs_idx, y_idx, observed_mod_indicator_idx = sample[0], sample[1], sample[2]
        ...
        assert torch.all(observed_mod_indicator_idx)
